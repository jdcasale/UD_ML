...
def explore_city_data(city_data):
    ...
    number_of_houses = np.size(housing_prices, axis=0)

    # Number of features?
    number_of_features = np.size(housing_features, axis=1)
    ...
    housing_prices_std_dev = np.std(housing_prices)
...
def fit_predict_model(city_data):
    ...
    print "Final Model: "
    print reg.fit(X, y)
    print "Best model parameters from grid search:  " + str(reg.best_params_)
